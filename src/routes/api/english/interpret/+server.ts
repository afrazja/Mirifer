...
import { checkJamieLine, signJamieLine, MAX_JAMIE_WORDS } from '$lib/server/jamie-line';
...
const ReplySchema = z.object({
	...
	jamieReply: z.string().max(400).nullable().optional()
}).strict();
const fields = ['choiceId', 'related', 'improved', 'noteEn', 'noteFa', 'jamieReply'];
const openAiSchema = { type: 'object', additionalProperties: false, required: fields, properties: {
	...
	noteEn: { type: ['string', 'null'] }, noteFa: { type: ['string', 'null'] }, jamieReply: { type: ['string', 'null'] }
} };
const geminiSchema = { type: 'OBJECT', required: fields, properties: {
	...
	noteEn: { type: 'STRING', nullable: true }, noteFa: { type: 'STRING', nullable: true }, jamieReply: { type: 'STRING', nullable: true }
} };
...
async function interpret(prompt: string, utterance: string) {
	const shape = '\nReturn only JSON with all six keys: {"choiceId": string or null, "related": boolean, "improved": string or null, "noteEn": string or null, "noteFa": string or null, "jamieReply": string or null}. Null means no correction. Never omit a key.';
	if (env.GEMINI_API_KEY) {
		const result = await requestProvider(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(env.GEMINI_MODEL || 'gemini-2.5-flash')}:generateContent`, { 'x-goog-api-key': env.GEMINI_API_KEY }, {
			...
			generationConfig: { temperature: 0, maxOutputTokens: 400, responseMimeType: 'application/json', responseSchema: geminiSchema, thinkingConfig: { thinkingBudget: 0 } }
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	const { stage, variant, utterance, previous } = parsed.data;
	...
	const jamieReply = choiceId === 'related' && stage !== 'recall' ? checkJamieLine(result.jamieReply, variant, previous) : null;
	if (!jamieReply) return json({ choiceId, correction });
	return json({ choiceId, correction, jamieReply, jamieSig: signJamieLine(jamieReply) });
};
